...
function removeUnwantedCharacters(str) {
  ...
  str = str.replace(/\s+/g, '');
  // Remove all non-printable characters (characters with ASCII codes < 32 and > 126)
  str = str.replace(/[^\x20-\x7E]/g, '');
  return str;
}

export function jsonFromDaisyTheme(clipboardString) {
	// This allows users to just use https://daisyui.com/theme-generator/
	// and copy-paste the tailwind.config.js
	// This function robustly extracts the theme-json based on the pattern {"primary"...}. Note this
	// strictly requires the primary color to be defined first.
	// else it returns null,
	const unwhitespacedInput = removeUnwantedCharacters(clipboardString);
	const regex = /{"primary".*?}/;
	const match = unwhitespacedInput.match(regex);

	if (match) {
			let theme =  match[0].replace(",}", "}"); // remove eventual superflous comma
			return JSON.parse(theme);
	} else {
		// in case there are other JSONs pasted, return them as JSOns
		// this function returns null if no json passed
		return jsonify(clipboardString);
	}
}
